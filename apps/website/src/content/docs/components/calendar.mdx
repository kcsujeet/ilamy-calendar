...
| `dragSnapInterval` | `1 \| 5 \| 10 \| 15 \| 30 \| 60` | `slotDuration` | Clock-aligned snapping interval for timed drags. Use `1` for minute-accurate movement without coarse snapping. Explicit values are independent of `slotDuration`. |
| `showDragTimeIndicator` | `boolean` | `true` | Show the live snap line and selected time while dragging over timed grids. |
| `renderDragTimeIndicator` | `(props: RenderDragTimeIndicatorProps) => ReactNode` | - | Replace the live drag-time indicator. Receives selected time, event, host range, progress, axis, resource, and view. |
...
## Drag Snapping and Time Indicator

Timed drag snapping is configured separately from the visual grid. This example
keeps hourly grid rows while allowing starts such as 18:45:

```tsx title="Quarter-Hour Drag Snapping"
<IlamyCalendar
  events={events}
  slotDuration={60}
  dragSnapInterval={15}
...
The event's leading edge snaps to the nearest clock boundary and the live line
shows the selected start time. In vertical day/week grids the line is horizontal;
in horizontal resource grids it follows the time axis and becomes vertical.
When `dragSnapInterval` is omitted, it follows `slotDuration`.
Set `showDragTimeIndicator={false}` to hide it, or use
`renderDragTimeIndicator` to replace its presentation.
Set `dragSnapInterval={1}` to allow movement to any whole minute, effectively
disabling coarser snapping.
