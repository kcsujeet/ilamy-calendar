...
const dayMaxEvents = 5 // Default max events per day
...
describe('WeekEventsLayer', () => {
	...
	test('leaves no gaps in cells if event can be rendered', () => {
		...
		const sundayWednesdayTop = Number.parseInt(
			sundayWednesdayEvent.style.top,
			10
		)
		const tuesdaySaturdayTop = Number.parseInt(
			tuesdaySaturdayEvent.style.top,
			10
		)
		const fridaySaturdayTop = Number.parseInt(fridaySaturdayEvent.style.top, 10)
		const thursdayTop = Number.parseInt(thursdayEvent.style.top, 10)
	...
	describe('Recurring Events', () => {
		...
		test('renders recurring events with proper positioning among non-recurring events', () => {
			...
			const nonRecurringTop = Number.parseInt(nonRecurringMonday.style.top, 10)
			const recurringTop = Number.parseInt(recurringMonday.style.top, 10)
		...
		test('renders multiple recurring events with different frequencies', () => {
			...
			const dailyTop = Number.parseInt(dailyEvent.style.top, 10)
			const weeklyTop = Number.parseInt(weeklyEvent.style.top, 10)
				...
				// Base recurring event with EXDATE to exclude the modified occurrence
				{
					...
					exdates: [
						dayjs()
							...
							.add(2, 'day')
							.hour(10)
							.toISOString(), // Exclude the original Tuesday occurrence
					],
				},
