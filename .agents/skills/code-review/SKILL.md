...
- [ ] **Readability conventions checked.** Every changed `.ts`/`.tsx` file was scanned for ternaries and ALL nested or multi-line ones flagged (Hard Rule), except shadcn `ui/` primitives and unmodified files. Inline multi-clause conditions, nested if/switch blocks, ternaries in JSX, and code that is hard to understand at a glance have also been flagged where present.
