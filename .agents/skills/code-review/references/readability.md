...
# Value-dispatch cascades that should be lookup maps: IIFEs assigned to a value
# (`const x = (() => { ... })()`) and if/else-if chains. Open each hit: an
# if-cascade or IIFE that maps ONE discriminant value to a returned value/JSX is a
# switch in disguise -> convert to a key-value map. (`(() =>` matches the IIFE/
# parenthesized-arrow form, NOT ordinary `() =>` callbacks.)
while IFS= read -r f; do [ -f "$f" ] || continue; grep -nE '\(\(\) =>|\belse +if *\(' "$f" | sed "s|^|$f:|"; done < /tmp/srcfiles.txt
```
...
- **No value-dispatch `if`-cascades or IIFEs that should be lookup maps.** A run of `if (x === 'a') return A; if (x === 'b') return B; ... return DEFAULT` (a plain cascade, an `if`/`else if` chain, or wrapped in an IIFE `const v = (() => { ... })()`) that maps ONE discriminant to a value is a `switch` in disguise → use a key-value lookup object. **Default to a plain value map, not builder functions** — `const MAP = { a: A, b: B }; const v = MAP[x] ?? DEFAULT`. Reach for builder functions (`Partial<Record<string, () => V>>`, looked up with `MAP[x]?.() ?? DEFAULT`) ONLY when a value genuinely needs one: it closes over a per-call argument, must be lazy (the unused branches are expensive or have side effects), or each case takes an argument. Plain values — strings, numbers, config objects, even JSX elements (an element is a cheap descriptor, not a render) — go straight in the map with no wrapping `() =>`. An unnecessary `() =>` wrapper is itself a finding (YAGNI: the function adds nothing). Canonical example: a header `title`/`content` IIFE branching on `view` → `titleByView`/`contentByView` as plain value maps. Type as `Partial<Record<...>>` so the `?? DEFAULT` fallback is type-justified.
