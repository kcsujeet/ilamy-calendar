...
The `@ilamy/calendar/plugins/recurrence` subpath requires **v2.0.0 or later**. On 1.x this import fails because recurrence was built into the core and no plugin subpath existed. Upgrade by following the [migration guide](/docs/help/migration-to-v2).
:::
...
An override **replaces** the occurrence it names: the series expansion skips that
occurrence, and the override is what renders — wherever it has been moved to,
including outside the range the series was expanded over. You do not also need an
`exdates` entry for it, and adding one changes nothing.
...
Entries are **ISO strings** (`exdates?: string[]`) keyed to the *original*
occurrence time, so they must be produced with `.toISOString()` — a `Date` never
matches.
...
const recurringEvent = {
  ...
  exdates: [
    dayjs('2025-08-18T09:00:00').toISOString(), // Skip this Monday
    dayjs('2025-08-25T09:00:00').toISOString()  // Skip this Monday too
  ]
}
```
